/*
  # Add Demo Challenge with Instructor Profile

  1. New Data
    - Ensure instructor profile exists for challenges
...
-- First, ensure we have an instructor profile that doesn't depend on auth.users
-- We'll temporarily disable the foreign key constraint, insert the profile, then re-enable it
...
-- Re-enable the foreign key constraint (but make it deferrable to avoid issues)
